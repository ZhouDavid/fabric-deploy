...
import (
	"Data_Bank/fabric-deploy-tools/sdk/constant"
	"Data_Bank/fabric-deploy-tools/sdk/model"
	"bytes"
	"fmt"
	"io/ioutil"
	"os"
...
client:
  ...
  tlsCerts:
	...
	client:
      key:
        path: %s/peerOrganizations/%s/users/%s@%s/tls/client.key
      cert:
        path: %s/peerOrganizations/%s/users/%s@%s/tls/client.crt
`
